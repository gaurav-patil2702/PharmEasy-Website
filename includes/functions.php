...
function singUp()
{
    ...
    if (isset($_POST['singUp'])) {
        ...
        if (empty($email) or empty($passwd) or empty($address) or empty($fname) or empty($m_no)) {
            $_SESSION['message'] = "empty_err";
        ...
        $data = query($query);
        $num = is_array($data) ? sizeof($data) : 0;
        for ($i = 0; $i < $count; $i++) {
        ...
        $query = "INSERT INTO user (email ,user_fname ,user_m_no , user_address,user_password ) VALUES('$email', '$fname' ,'$m_no','$address' ,'$passwd')";
...
function total_price($data)
{
    ...
    $num = sizeof($data);
    for ($i = 0; $i < $num; $i++) {
        // Check if 'quantity' key exists in the array before accessing it
        if (isset($_SESSION['cart'][$i]['quantity'])) {
...
function add_cart($item_id)
{
    ...
    if (empty($user_id)) {
        ...
        if (isset($_SESSION['cart'])) {
            ...
            for ($i = 0; $i < $num; $i++) {
                for ($j = $i + 1; $j < $num; $j++) {
                    if (isset($_SESSION['cart'][$i]['item_id'], $_SESSION['cart'][$j]['item_id'])) {
