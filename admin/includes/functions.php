...
function all_users()
{
    $query = "SELECT user_id ,user_fname ,user_m_no ,email ,user_address FROM user";
    $data = query($query);
    return $data;
}
...
function edit_user($id)
{
    if (isset($_POST['update'])) {
        ...
        if (empty($email) or empty($address) or empty($fname) or empty($m_no)) {
        ...
        if ($check == 0) {
            $query = "UPDATE user SET email='$email' ,user_fname='$fname' ,user_m_no='$m_no' ,user_address='$address' WHERE user_id= '$id'";
            single_query($query);
            get_redirect("customers.php");
        } else {
...
{
    $query = "SELECT user_id ,user_fname ,user_m_no ,email ,user_address FROM user WHERE user_id=$id";
    $data = query($query);
    return $data;
}
...
function search_user()
{
    if (isset($_GET['search_user'])) {
        ...
        $query = "SELECT user_id ,user_fname ,user_m_no ,email ,user_address FROM user WHERE email='$email'";
        $data = query($query);
        if ($data) {
            return $data;
        } else {
    ...
    if (isset($_POST['update'])) {
        ...
        if ($check == 0) {
            $query = "UPDATE item SET item_title='$name', item_brand='$brand', item_cat='$cat',
                      item_details='$details', item_tags='$tags', item_image='$image',
                      item_quantity='$quantity', item_price='$price' WHERE item_id= '$id'";
            $run = single_query($query);
            get_redirect("products.php");
        } else {
...
function add_item()
{
    if (isset($_POST['add_item'])) {
        ...
        if ($check == 0) {
            $query = "INSERT INTO item (item_title, item_brand, item_cat, item_details, item_tags, item_image, item_quantity, item_price) 
...
function edit_admin($id)
{
    if (isset($_POST['admin_update'])) {
        ...
        $m_no = trim($_POST['admin_lname']);
        $email = trim(strtolower($_POST['admin_email']));
        $password = trim($_POST['admin_password']);
        $check = check_email_admin($email);
        if ($check == 0) {
            $query = "UPDATE admin SET admin_email='$email' ,admin_fname='$fname' ,admin_lname='$m_no' ,admin_password='$password'  WHERE admin_id= '$id'";
            single_query($query);
...
function add_admin()
{
    if (isset($_POST['add_admin'])) {
        ...
        if ($check == 0) {
            ...
            VALUES ('$fname','$m_no','$email','$password')";
            single_query($query);
            get_redirect("admin.php");
        } else {
            $_SESSION['message'] = "emailErr";
